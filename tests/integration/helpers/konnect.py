...
class KonnectHelper:
    ...
    def check_product_document_structure(self, product_id: str) -> None:
        response = requests.get(f"{self.test_server_url}/v2/api-products/{product_id}")
        api_product_id = response.json()["id"]

        response = requests.get(f"{self.test_server_url}/v2/api-products/{api_product_id}/documents")
    ...
    def get_portal_product_version_by_product_version_id(self, portal_id: str, product_version_id: str) -> Dict[str, Any]:
        response = requests.get(f"{self.test_server_url}/v2/portals/{portal_id}/product-versions")
    ...
    def get_api_product_version_by_name(self, api_product_id: str, version: str) -> Dict[str, Any]:
        response = requests.get(f"{self.test_server_url}/v2/api-products/{api_product_id}/product-versions")
        product_version = next(pv for pv in response.json()["data"] if pv["name"] == version)
        return product_version

    def get_portal_and_product(self, portal_name: str) -> (Dict[str, Any], Dict[str, Any]):
    ...
    def get_api_product(self) -> Dict[str, Any]:
        response = requests.get(f"{self.test_server_url}/v2/api-products")
        api_product = response.json()["data"][0]
        return api_product

    def get_portal_by_name(self, portal_name: str) -> Dict[str, Any]:
        response = requests.get(f"{self.test_server_url}/v2/portals")
